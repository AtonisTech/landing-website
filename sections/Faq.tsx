...
const Faq = () => {
    ...
    return (
        <section className="bg-gradient-to-b from-brand-30/80 to-brand-30 py-16 md:py-24 pt-8 md:pt-8">
                        ...
                        <dl className="leading-relaxed text-sm">
                            {
                                data.faqs.map((item,ind)=>{
                                    ...
                                    return <React.Fragment key={ind}>
                                        <dt onClick={() => { setSelectedQues(ind) }} className={`text-slate-900 font-semibold text-left text-base flex justify-between w-full items-center cursor-pointer ${ind !== 0 &&"mt-8"}`}>
                                            {item.question}
                                        ...
                                        </dt>
                                        {
                                            isSelected && item.answers.map((ans,i)=>{
                                                return <dd key={`${ind}.${i}`} className="mt-3" dangerouslySetInnerHTML={{ __html: ans }}></dd>
                                            })
                                        }
                                    </React.Fragment>
                                })
